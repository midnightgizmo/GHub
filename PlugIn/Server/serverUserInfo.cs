...
namespace GHub.client.server
{
	...
	public class serverUserInfo
	{
		...
		public string parseNickFromMyInfo(string myINFO)
		{
			string[] infoSplitUp = myINFO.Split('$');
			int position;
			// now looks like following
			// "Gizmo abc<++ V:0.668,M:A,H:1/0/1,S:4>"
			infoSplitUp[2] = infoSplitUp[2].Substring(4);
			...
			return infoSplitUp[2].Substring(0,  infoSplitUp[2].Length  - (infoSplitUp[2].Length - position)  );
